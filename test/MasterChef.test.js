...
contract('MasterChef', ([alice, bob, carol, referrer, treasury, dev, fee, owner]) => {
    beforeEach(async () => {
        ...
        this.referral = await BullyReferral.new({ from: owner });
        this.chef = await MasterChef.new(this.bully.address, '100', '1000', { from: owner });

        await this.bully.transferOwnership(this.chef.address, { from: owner });
        await this.referral.updateOperator(this.chef.address, true, { from: owner });
        await this.chef.setBullyReferral(this.referral.address, { from: owner });
